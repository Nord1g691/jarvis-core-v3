/* JARVIS V3.0.25 — dynamic Home Assistant domain cards. */
...
if(Panel&&!Panel.prototype.__jarvisDomainsInstalled){
  ...
  Panel.prototype._loadPrefs=function(){
    ...
    const extra={lights:true,climate:true,media:true,covers:true,switches:true};
    p.cards={...extra,...p.cards};
    for(const k of ['lights','climate','media','covers','switches'])if(!p.order.includes(k))p.order.push(k);
    return p;
  };
  ...
  Panel.prototype._renderDomain=function(cardId,domain){
    ...
    states.forEach(s=>{const row=document.createElement('div');row.className='jarvis-entity';const name=s.attributes?.friendly_name||s.entity_id;let detail=s.state;let actions=[];
  ...
  Panel.prototype._renderCards=function(){baseRender.call(this);const labels={lights:'💡 Éclairage',climate:'🌡️ Clim / Chauffage',media:'📺 Médias',covers:'🪟 Volets / Ouvertures',switches:'🔌 Prises / Switches'};const box=this.shadowRoot?.getElementById('cards');if(!box)return;[...box.querySelectorAll('.row')].forEach((r,i)=>{const k=this._prefs.order[i];if(labels[k])r.querySelector('span').textContent=labels[k]})};
