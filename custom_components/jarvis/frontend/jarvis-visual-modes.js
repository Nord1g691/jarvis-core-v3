/* JARVIS V4.0.0 — selectable visual skins for native V4 HUD. */
...
if(Panel&&!Panel.prototype.__jarvisVisualModesInstalled){
 ...
 const MODES=[
  {key:'classic',name:'Classic HUD',tag:'ORIGINAL',desc:'Le design JARVIS V4 standard.'},
 ...
 :host([data-visual-mode="sentinel"]) .core{border:1px solid #ff4d451c}
 ...
 Panel.prototype._jarvisInstallCornerVisualPicker=function(){const root=this._core?.shadowRoot;if(!root||root.getElementById('jarvisVisualButton'))return;const btn=document.createElement('button');btn.id='jarvisVisualButton';btn.type='button';btn.title='Changer le design JARVIS';btn.textContent='◈';btn.style.cssText='position:fixed;right:12px;top:12px;z-index:10002;width:36px;min-width:36px;height:34px;min-height:34px;margin:0;padding:0;border-radius:9px;font-size:18px;backdrop-filter:blur(12px)';const pop=document.createElement('div');pop.id='jarvisVisualPopover';pop.hidden=true;pop.innerHTML='<div class="jv-pop-title">DESIGN JARVIS</div>'+MODES.map(m=>`<button type="button" data-jv-quick-theme="${m.key}">${esc(m.name)}</button>`).join('');pop.style.cssText='position:fixed;right:12px;top:54px;z-index:10003;width:200px;padding:8px;border:1px solid #00eaff33;border-radius:10px;background:#020711ee;backdrop-filter:blur(18px);box-shadow:0 12px 35px #0009';root.append(btn,pop);btn.onclick=()=>{pop.hidden=!pop.hidden};pop.querySelectorAll('[data-jv-quick-theme]').forEach(b=>b.onclick=()=>{this._jarvisSetVisualMode(b.dataset.jvQuickTheme);pop.hidden=true});this._jarvisSyncVisualPicker()};
 Panel.prototype._v4RenderVisualModes=function(){const panel=this.shadowRoot?.getElementById('settingsPanel');if(!panel)return;let section=panel.querySelector('[data-v4-section="visual-modes"]');if(!section){section=document.createElement('div');section.className='section';section.dataset.v4Section='visual-modes';section.innerHTML='<div class="title">◈ APPARENCE / DESIGN JARVIS</div><div class="note">Choisis le rendu visuel du Core V4.</div><div class="jv-theme-gallery"></div>';panel.insertBefore(section,panel.querySelector('.actions'));const s=document.createElement('style');s.textContent='.jv-theme-gallery{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:9px}.jv-theme-card{display:grid!important;gap:2px!important;text-align:left!important;padding:9px!important;min-height:72px!important;margin:0!important;background:#020b14!important}.jv-theme-card.active{border-color:#39ff88!important;box-shadow:inset 0 0 18px #39ff8810}.jv-theme-card strong{font-size:10px}.jv-theme-card small{font-size:7px;letter-spacing:1px;opacity:.6}.jv-theme-card em{font-size:8px;font-style:normal;opacity:.58}@media(max-width:650px){.jv-theme-gallery{grid-template-columns:1fr}}';this.shadowRoot.appendChild(s)}const active=mode(),gallery=section.querySelector('.jv-theme-gallery');gallery.innerHTML=MODES.map(m=>`<button type="button" class="jv-theme-card ${active===m.key?'active':''}" data-jv-theme="${m.key}"><strong>${esc(m.name)}</strong><small>${esc(m.tag)}</small><em>${esc(m.desc)}</em></button>`).join('');gallery.querySelectorAll('[data-jv-theme]').forEach(b=>b.onclick=()=>{this._jarvisSetVisualMode(b.dataset.jvTheme);this._v4RenderVisualModes()})};
