/* JARVIS Core V3.0.13 — persistent memory UI and safe HUD spacing. */
(() => {
  const boot = () => {
    ...
    style.textContent = `
      ...
      @media(max-width:650px){.j313-memory-btn{right:60px}}
    `;
    ...
    const render=async()=>{try{const data=await api('/api/jarvis/memory?limit=50');list.innerHTML='';const items=data.memories||[];if(!items.length){list.innerHTML='<div class="j313-empty">Aucune mémoire enregistrée.</div>';return;}items.forEach(m=>{const e=document.createElement('div');e.className='j313-item';e.innerHTML=`<button type="button" data-q="${String(m.text).replace(/"/g,'&quot;')}">OUBLIER</button><span>${String(m.text).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</span><small>${m.category||'personal'}</small>`;e.querySelector('button').onclick=async()=>{await api('/api/jarvis/memory?q='+encodeURIComponent(m.text),{method:'DELETE'});render()};list.appendChild(e)});}catch(err){list.innerHTML='<div class="j313-empty">Mémoire indisponible : '+String(err.message||err)+'</div>';}};
    ...
    render();
    // Keep the Core from covering the operational label or the cards below it.
    const core = root.querySelector('.core');
    if(core){core.style.marginTop='8px';core.style.marginBottom='8px';core.style.maxHeight='min(68vw,420px)';core.style.maxWidth='min(68vw,420px)';}
    const label = root.getElementById('state');
    if(label){label.style.bottom='3%';label.style.zIndex='20';}
  };
