...
(async () => {
  ...
  const addBar = (host) => {
    ...
    if (!root.getElementById('jarvisTextBarStyle')) {
      ...
      style.textContent = `
        ...
        @media(max-width:650px){.app{padding-bottom:72px}#jarvisTextBar{left:10px;right:10px;bottom:10px;gap:6px;padding:6px}#jarvisStateBridge{left:10px;right:10px;bottom:56px;font-size:10px;letter-spacing:2px}#jarvisTextBar input{font-size:13px}#jarvisTextBar button{min-width:78px}}
      `;
  ...
  if (!C.prototype.__jarvisConversationStabilizerV3) {
    ...
    C.prototype.toggleConversation = function (...args) {
      ...
      this.__jarvisListenWindowsRemaining = wasActive ? 0 : 2;
  ...
  const installStateBridge = () => {
    ...
    const wrapped = function (text, color) {
      ...
      if (core) core.classList.remove('state-listening','state-thinking','state-speaking','state-searching','state-error');
      if (status) {
