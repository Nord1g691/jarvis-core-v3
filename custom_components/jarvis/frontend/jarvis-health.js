/* JARVIS Core V3.0.28 — global health score card + subtle core health ring. */
...
if(Panel&&!Panel.prototype.__jarvisHealthInstalled){
 ...
 Panel.prototype._jarvisHealthRing=function(){
  ...
  if(!ring){ring=document.createElement('div');ring.className='jarvis-health-ring';ring.innerHTML='<div class="jarvis-health-ring-track"></div><div class="jarvis-health-ring-progress"></div><div class="jarvis-health-ring-dot"></div>';core.prepend(ring)}
  if(!root.getElementById('jarvisHealthRingStyle')){const s=document.createElement('style');s.id='jarvisHealthRingStyle';s.textContent=`
 ...
 Panel.prototype._jarvisApplyHealthToCore=function(score,level){const ring=this._jarvisHealthRing();if(!ring)return;const safe=Math.max(0,Math.min(100,Number(score)||0));ring.style.setProperty('--jv-health-score',String(safe));ring.style.setProperty('--jv-health-color',levelColor(level));ring.dataset.level=String(level||'unknown');ring.title=`Santé JARVIS / Maison : ${safe}/100 · ${levelLabel(level)}`};
 Panel.prototype._jarvisLoadHealth=async function(){
  const root=this._core?.shadowRoot,scoreEl=root?.getElementById('jarvisHealthScore'),levelEl=root?.getElementById('jarvisHealthLevel'),bars=root?.getElementById('jarvisHealthBars'),issues=root?.getElementById('jarvisHealthIssues');if(!scoreEl||!levelEl||!bars||!issues)return;
  try{const d=await this._hass.callApi('GET','jarvis/diagnostics');const h=d?.health||{},score=Number(h.score||0),subs=h.sub_scores||{};scoreEl.textContent=String(score);levelEl.textContent=levelLabel(h.level);levelEl.dataset.level=h.level||'warning';this._jarvisApplyHealthToCore(score,h.level);bars.innerHTML=Object.entries(subs).map(([k,v])=>`<div class="jv-health-row"><span>${LABELS[k]||k.toUpperCase()}</span><div><i style="width:${Math.max(0,Math.min(100,Number(v)||0))}%"></i></div><b>${Number(v)||0}</b></div>`).join('');const list=Array.isArray(h.issues)?h.issues:[];issues.innerHTML=list.length?list.map(x=>`<div class="jv-health-issue ${String(x.severity||'info')}"><b>${String(x.area||'system').toUpperCase()}</b><span>${String(x.message||'')}</span></div>`).join(''):'<div class="jv-health-ok">Aucun problème prioritaire détecté.</div>'}catch(e){scoreEl.textContent='--';levelEl.textContent='INDISPONIBLE';issues.textContent='Health Score indisponible : '+e.message;this._jarvisApplyHealthToCore(0,'critical')}
 };
