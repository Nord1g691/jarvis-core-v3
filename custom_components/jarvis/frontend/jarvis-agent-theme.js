/* JARVIS V4.0.0 — agent-aware HUD theme and voice bridge. */
...
if(Panel&&!Panel.prototype.__jarvisAgentThemeInstalled){
 ...
 Panel.prototype._jarvisApplyAgentTheme=function(labelOverride){
  ...
  const label=(labelOverride||this._jarvisAgentLabel()||'').trim(),t=this._jarvisThemeForAgent(label);this._jarvisActiveTheme=t;this._jarvisActiveAgentLabel=label;
  ...
  let style=root.getElementById('jarvisAgentThemeStyle');if(!style){style=document.createElement('style');style.id='jarvisAgentThemeStyle';style.textContent=`
   ...
   .satellite{background:var(--jarvis-agent)!important;box-shadow:0 0 10px var(--jarvis-agent),0 0 18px var(--jarvis-agent-soft)!important}
   ...
   button,select,.commandInput input{border-color:var(--jarvis-agent-soft)!important;color:var(--jarvis-agent-text)!important}.fill{background:var(--jarvis-agent)!important;box-shadow:0 0 12px var(--jarvis-agent),0 0 18px var(--jarvis-agent-soft)!important}
   .core.state-listen .ring,.core.state-listen .orbit{border-color:#39ff88!important;box-shadow:0 0 16px #39ff8899,inset 0 0 12px #39ff8844!important}.core.state-listen .voiceBar{background:#39ff88!important;box-shadow:0 0 10px #39ff88,0 0 22px #39ff8899!important}.core.state-listen .glow{background:radial-gradient(circle,#fff 0 4%,#39ff88 14%,#39ff8899 35%,#39ff8844 56%,transparent 75%)!important;box-shadow:0 0 48px #39ff88,0 0 120px #39ff8899!important}
   .core.state-think .ring{border-color:#ffb000!important;box-shadow:0 0 16px #ffb00099,inset 0 0 12px #ffb00044!important}.core.state-think .glow{background:radial-gradient(circle,#fff 0 4%,#ffb000 14%,#ffb00099 35%,#ffb00044 56%,transparent 75%)!important;box-shadow:0 0 50px #ffb000,0 0 125px #ffb00099!important}
   .core.state-search .ring,.core.state-search .orbit{border-color:#00eaff!important;box-shadow:0 0 16px #00eaff99,inset 0 0 12px #00eaff44!important}.core.state-search .glow{background:radial-gradient(circle,#fff 0 4%,#00eaff 14%,#00eaff99 35%,#00eaff44 56%,transparent 75%)!important;box-shadow:0 0 48px #00eaff,0 0 120px #00eaff99!important}
   .core.state-speak .ring,.core.state-speak .orbit{border-color:#b56cff!important;box-shadow:0 0 17px #b56cff99,inset 0 0 12px #b56cff44!important}.core.state-speak .voiceBar{background:#b56cff!important;box-shadow:0 0 11px #b56cff,0 0 24px #b56cff99!important}.core.state-speak .glow{background:radial-gradient(circle,#fff 0 4%,#b56cff 14%,#b56cff99 35%,#b56cff44 56%,transparent 75%)!important;box-shadow:0 0 52px #b56cff,0 0 130px #b56cff99!important}
  `;root.appendChild(style)}
 };
 Panel.prototype._jarvisInstallAgentVoice=function(){if(!this._core)return;const panel=this;this._core.__jarvisThemePanel=this;this._core.speak=function(text){if(this.muted)return Promise.resolve();return new Promise(resolve=>{const t=panel._jarvisActiveTheme||DEFAULT;this.setState('JARVIS PARLE','#b56cff');const u=new SpeechSynthesisUtterance(text);u.lang='fr-FR';u.rate=t.rate||.92;u.pitch=t.pitch||1;u.volume=Math.max(0,Math.min(1,Number(panel._prefs?.volume??70)/100));const voices=panel._jarvisFrenchVoices();if(voices.length)u.voice=voices[Math.abs(Number(t.voice)||0)%voices.length];u.onend=u.onerror=()=>{this.setState('OPÉRATIONNEL',t.color);resolve()};speechSynthesis.cancel();speechSynthesis.speak(u)})}};
 const baseBoot=Panel.prototype._bootCore;Panel.prototype._bootCore=async function(){await baseBoot.call(this);this._jarvisApplyAgentTheme();this._jarvisInstallAgentVoice();const root=this._core?.shadowRoot,select=root?.getElementById('pipelineSelect');if(select&&!select.__jarvisThemeBound){select.addEventListener('change',()=>queueMicrotask(()=>{this._jarvisApplyAgentTheme();this._jarvisInstallAgentVoice()}));select.__jarvisThemeBound=true}this._jarvisAgentThemeObserver?.disconnect();if(select){const mo=new MutationObserver(()=>this._jarvisApplyAgentTheme());mo.observe(select,{childList:true,subtree:true,attributes:true});this._jarvisAgentThemeObserver=mo}};
