...
(() => {
  const boot = () => {
    ...
    style.textContent = `
      ...
      /* Keep the Core compact enough that the cards and status have their own space. */
    ...
    const render=async()=>{try{const data=await api('/api/jarvis/memory?limit=50');list.innerHTML='';const items=data.memories||[];if(!items.length){list.innerHTML='<div class="j313-empty">Aucune mémoire enregistrée.</div>';return;}items.forEach(m=>{const e=document.createElement('div');e.className='j313-item';e.innerHTML=`<button type="button">OUBLIER</button><span>${String(m.text).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</span><small>${m.category||'personal'}</small>`;e.querySelector('button').onclick=async()=>{await api('/api/jarvis/memory?q='+encodeURIComponent(m.text),{method:'DELETE'});render()};list.appendChild(e)});}catch(err){list.innerHTML='<div class="j313-empty">Mémoire indisponible : '+String(err.message||err)+'</div>';}};
