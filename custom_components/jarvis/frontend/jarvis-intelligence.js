...
if(Panel&&!Panel.prototype.__jarvisIntelligenceInstalled){
 ...
 const PIN_KEY='jarvis_pinned_entities_v326';
 ...
 Panel.prototype._jarvisLoadActivity=async function(){const root=this._core?.shadowRoot,log=root?.getElementById('jarvisActivityLog');if(!log)return;log.style.display='block';log.textContent='Chargement…';try{const token=this._hass?.auth?.data?.access_token;if(!token)throw Error('auth indisponible');const start=new Date(Date.now()-6*3600*1000).toISOString();const r=await fetch('/api/logbook/'+encodeURIComponent(start),{headers:{Authorization:'Bearer '+token}});if(!r.ok)throw Error('HTTP '+r.status);const data=await r.json(),ignored=this._jarvisIgnored();const items=(Array.isArray(data)?data:[]).filter(x=>!ignored.has(x.entity_id)).slice(-20).reverse();log.innerHTML=items.length?items.map(x=>`${new Date(x.when||x.time_fired||Date.now()).toLocaleTimeString()} · ${x.name||x.entity_id||'Événement'} · ${x.message||x.state||''}`).join('<br>'):'Aucune activité récente.'}catch(e){log.textContent='Activité indisponible : '+e.message}};
