...
if(Panel&&!Panel.prototype.__jarvisDomainsInstalled){
  ...
  Panel.prototype._domainStates=function(domain){const lightingSwitch=s=>s.entity_id==='switch.eclairage_garage'||/^switch\.smart_switch_(2005127735948155850448e1e91bc62e|2112033358624651859048e1e97fa305|2105100715876890850948e1e970d82a|2103188752458451855248e1e96523aa|2103188024426251855248e1e9652292)_outlet$/.test(s.entity_id||'');const domains=domain==='climate'?['climate','fan']:[domain];return Object.values(this._hass?.states||{}).filter(s=>(domain==='light'&&lightingSwitch(s))||(domains.some(d=>s.entity_id?.startsWith(d+'.'))&&!(domain==='switch'&&lightingSwitch(s)))).sort((a,b)=>String(a.attributes?.friendly_name||a.entity_id).localeCompare(String(b.attributes?.friendly_name||b.entity_id),'fr'))};
  ...
  Panel.prototype._entityVisual=function(s){const domain=s.entity_id.split('.')[0],attrs=s.attributes||{},text=String(`${s.entity_id} ${attrs.friendly_name||''}`).toLowerCase();if(domain==='light'&&s.state==='on'){const rgb=attrs.rgb_color;return{active:true,color:Array.isArray(rgb)?`rgb(${rgb[0]},${rgb[1]},${rgb[2]})`:'#ffd60a'}}if(domain==='switch'&&s.state==='on'&&/éclairage|eclairage|guirlande|lampe|allée|allee/.test(text))return{active:true,color:'#ffd60a'};if(domain==='climate'&&s.state!=='off'&&s.state!=='unavailable'){const action=String(attrs.hvac_action||s.state).toLowerCase();return{active:true,color:action.includes('heat')?'#ff5a45':action.includes('cool')?'#38bdf8':action.includes('dry')?'#ffb000':'#7dd3fc'}}if(domain==='fan'&&s.state==='on')return{active:true,color:'#62d8ff'};if(domain==='switch'&&s.state==='on'&&/piscine|pool|filtration/.test(text))return{active:true,color:'#249dff'};if((domain==='switch'||domain==='media_player')&&['on','playing'].includes(s.state))return{active:true,color:'#39ff88'};return{active:false,color:'#00eaff'}};
