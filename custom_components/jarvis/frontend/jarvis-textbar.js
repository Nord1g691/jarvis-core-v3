...
(async () => {
  ...
  // Also retry after the custom element has rendered its shadow DOM.
  const retry = setInterval(() => {
    scan();
    const hosts = [...document.querySelectorAll('jarvis-core-hud')];
    if (hosts.length && hosts.every(h => h.shadowRoot?.getElementById('jarvisTextBar'))) clearInterval(retry);
  }, 250);
