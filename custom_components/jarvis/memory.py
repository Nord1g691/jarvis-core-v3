...
def add_memory(hass: HomeAssistant, text: str, category: str = "personal") -> dict[str, Any]:
    ...
    for item in items:
        if item.get("text", "").casefold() == text.casefold():
            ...
            _save(hass, items)
            return item
    item = {"id": f"m-{int(datetime.now(timezone.utc).timestamp()*1000)}", "text": text, "category": category or "personal", "created_at": now, "updated_at": now}
    items.append(item)
    _save(hass, items)
    return item
...
def search_memories(hass: HomeAssistant, query: str = "", limit: int = 8) -> list[dict[str, Any]]:
    items = _load(hass)
    query = " ".join(query.split()).strip().casefold()
    if not query:
